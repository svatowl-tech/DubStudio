...
### Сборка приложения
Для локальной сборки готового инсталлятора (приложения) используйте стандартные команды:

*   **Windows / Linux / macOS:** `npm run build:electron` (или используйте `npx electron-builder`)

### 🤖 Автоматическая сборка и релизы (GitHub Actions)
В проекте настроен CI/CD Pipeline для автоматической сборки. Каждый Push в ветку `main` и создание с Pull Request будет запускать тестовую сборку приложения на Windows, Linux и macOS.

Для автоматического создания **Релиза (Release)** с готовыми инсталляторами:
1. Зафиксируйте все изменения и обновите версию приложения в `package.json`.
2. Создайте новый тег версии с префиксом `v` (например: `v1.0.1`)
   ```bash
   git tag v1.0.1
   git push origin v1.0.1
   ```
3. GitHub Actions автоматически соберет приложение под все операционные системы и выложит готовые файлы установщиков (exe, dmg, AppImage и т.д.) во вкладку **Releases** на GitHub.
